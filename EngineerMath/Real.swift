...
public func += ( lhs: inout Real, rhs: Real) { lhs = lhs + rhs }
...
public protocol BasicOperationType : SignedNumeric, Comparable {
...
public struct Real : CustomStringConvertible, BasicOperationType {
    ...
    public var magnitude: Real { return self.abs }
    
    public typealias Magnitude = Real
	...
	private mutating func createUserPoint() {
		...
		if exponent < 0 {
			if Digit(-exponent) > valuePrecision * Digit(Real.NumDigits) {
				exponent += Int(valuePrecision * Digit(Real.NumDigits)) - 1
    ...
    private static func ipower<T:BasicOperationType>(_ number: T, power: Int) -> T {
        var Z : T = 0
