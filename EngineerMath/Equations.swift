...
class Equations {
	...
	static func curveFit (_ x: Vector, y: Vector, order: Int) -> Vector? {
		let size = order + 1
		var xData = [Double](repeating: 0, count: 2*order+1)
		var yData = [Double](repeating: 0, count: size)
		var b = x
		var c = y
	...
	static func solveUsingGaussianElimination (_ a: Matrix, y: Vector) -> Vector? {
		let n = y.count-1
		let b = a
		let w = y
